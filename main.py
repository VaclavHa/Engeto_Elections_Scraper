...
def input_check():
    ...
    if len(sys.argv) != 3:
        print("Requiered (3) parameters not entered. Run again.")
        exit()

    # to check if requiered URL has been entered
    if "https://volby.cz/pls/ps2017nss/ps32" not in sys.argv[1]:
        print("URL parameter not valid. Enter valid URL. Run again.")
        exit()

    # to check if required outpout file name has been entered
    if not sys.argv[2].endswith(".csv"):
        print("Invalid output file name. Run again.")
        exit()

    #to check if the output file already exist
    if os.path.isfile(sys.argv[2]):
        action = input("File already exist. Do you wish to overwrite the file?\n"
         "Enter 'y' to accept. Enter any key to exit. ")
        if action.lower() != "y":
            print("Terminating... Run again")
            exit()

    print("Program is running... This may take a moment...\n"
